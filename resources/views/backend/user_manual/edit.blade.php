...
@section('content')
    ...
    <div class="main-container">
        ...
        <div class="mx-5 p-4 shadow">
            <h6 class="text-uppercase fw-semibold text-center">Update User Manuals</h6>
            <form action="{{ route('user-manuals.update',$user_manual->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="row">
                    <!--title start-->
                    <div class="col-12 mb-3 login-field">
                        ...
                        <input type="text" name="title" id="title" value="{{@$user_manual->title ?? old('title') }}"
                    ...
                    <div class="form-group mb-3 col-12 login-field">
                        ...
                        <div>
                            <input type="file" id="file" name="file" data-height="150"
                                ...
                                data-default-file="{{@$user_manual->file ? asset('upload/user-manual/',$user_manual->file) : ''}}" />
                            @error('file')
                                <span class="text-danger" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="form-check form-switch d-flex align-items-center gap-2 languge-content mb-3">
                        <div>
                            <input class="form-check-input language-btn ms-0 mt-0" name="status" type="checkbox"
                                role="switch" {{$user_manual->status == 1 ? 'checked' : ''}} />
                        </div>
                        <p class="">Status</p>
                    </div>
                    <!--status end-->
